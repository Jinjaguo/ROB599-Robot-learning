...
def process_data_single_step(collected_data, batch_size=500):
    ...
    train_dataset, val_dataset = random_split(dynamics_dataset, [train_size, val_size])
...
def process_data_multiple_step(collected_data, batch_size=500):
    """
    Process the collected data and returns a DataLoader for train and one for validation.
    The data provided is a list of trajectories (like collect_data_random output).
    ...
    val_loader = None
    # --- Your code here
    dynamics_dataset = MultiStepDynamicsDataset(collected_data)
    ...
    def __getitem__(self, item):
        ...
        for data in self.data:
            for i in range(self.trajectory_length):
                ...
                self.dataset.append({"state": torch.as_tensor(current_state, dtype=torch.float32),
                                     "action": torch.as_tensor(current_action, dtype=torch.float32),
    ...
    def __getitem__(self, item):
        ...
        for data in self.data:
            for t in range(self.trajectory_length):
                xt = data['states'][t]
                ut = data['actions'][t:t + self.num_steps]
                xn = data['states'][t + 1:t + self.num_steps + 1]
                self.dataset.append({
                    "state": torch.as_tensor(xt, dtype=torch.float32),
                    "action": torch.as_tensor(ut, dtype=torch.float32),
                    "next_state": torch.as_tensor(xn, dtype=torch.float32)
                })
        sample = self.dataset[item]
        # ---
        return sample
...
class SE2PoseLoss(nn.Module):
    ...
    def __init__(self, block_width, block_length):
        ...
        self.w = block_width
        self.l = block_length
        self.rotation_scale = ((self.l ** 2 + self.w ** 2) / 12) ** 0.5
        self.loss_fn = nn.MSELoss(reduction='mean')

    def forward(self, pose_pred, pose_target):
        se2_pose_loss = None
        # --- Your code here
        translation_loss = self.loss_fn(pose_pred[:, 0], pose_target[:, 0]) + self.loss_fn(pose_pred[:, 1],
                                                                                           pose_target[:, 1])
        rotation_loss = self.rotation_scale * self.loss_fn(pose_pred[:, 2], pose_target[:, 2])

        se2_pose_loss = translation_loss + rotation_loss
        # ---
...
class SingleStepLoss(nn.Module):
    ...
    def forward(self, model, state, action, target_state):
        ...
        single_step_loss = self.loss(new_state, target_state)
        # ---
...
class MultiStepLoss(nn.Module):
    ...
    def forward(self, model, state, actions, target_states):
        ...
        for step in range(num_steps):
            ...
            step_loss = (self.discount ** step) * self.loss(state, target_states[:, step, :])
            multi_step_loss += step_loss
        # ---
...
def collision_detection(state):
    ...
    rotated_w = half_size * torch.abs(torch.cos(theta)) + half_size * torch.abs(torch.sin(theta))
    rotated_l = half_size * torch.abs(torch.sin(theta)) + half_size * torch.abs(torch.cos(theta))

    corners_x = pos[:, 0].unsqueeze(1) + torch.stack([rotated_w, rotated_w, -rotated_w, -rotated_w], dim=1)
    corners_y = pos[:, 1].unsqueeze(1) + torch.stack([rotated_l, -rotated_l, -rotated_l, rotated_l], dim=1)
    ...
    in_collision = torch.any(x_collides & y_collides, dim=1).float()
    # ---
    return in_collision
