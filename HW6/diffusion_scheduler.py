...
class DDPMScheduler:
    ...
    def add_noise(
        ...
        ################################
        #######  Your code here  #######
        sqrt_alpha_prod = torch.sqrt(alphas_cumprod[timesteps])                      # shape: (B,)
        sqrt_one_minus_alpha_prod = torch.sqrt(1.0 - alphas_cumprod[timesteps])      # shape: (B,)
    ...
    def step(
        ...
        ################################
        #######  Your code here  #######
        sqrt_alpha_bar_t   = torch.sqrt(alpha_prod_t)            # √( \barα_t )
        sqrt_one_minus_bar = torch.sqrt(1.0 - alpha_prod_t)      # √(1 - \barα_t)
        pred_original_sample = (sample - sqrt_one_minus_bar * pred_noise) / sqrt_alpha_bar_t
        #    ↳ 对应 x0 = 1/√(barα_t) * ( x_t - √(1-barα_t) * ε )

        one_minus_bar_t = 1.0 - alpha_prod_t                      # (1 - \barα_t)
        current_sample_coeff = (torch.sqrt(current_alpha_t)     # √α_t
                              * (1.0 - alpha_prod_t_prev)   # * (1 - \barα_{t-1})
                              / one_minus_bar_t)            # / (1 - \barα_t)
        pred_original_sample_coeff = (torch.sqrt(alpha_prod_t_prev)   # √( \barα_{t-1} )
                              * current_beta_t              # * β_t
                              / one_minus_bar_t)            # / (1 - \barα_t)
#    ↳ 对应 x_{t-1} = coeff_xt * x_t + coeff_x0 * x0
